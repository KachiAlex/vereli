...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      if (!user.userId) {
        console.error('[me] user.userId missing from token payload:', JSON.stringify(user));
      ...
      console.error('[me] GET error:', err.message, err.stack);
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    try {
      ...
      if (row && row.tenant_id && (company !== undefined || logoUrl !== undefined || primary !== undefined)) {
        const tFields = [];
        const tValues = [];
      ...
      console.error('[me] PUT/PATCH error:', err.message, err.stack);
      sendJson(res, 500, { error: 'Internal server error' });
    }
    return;
  }
