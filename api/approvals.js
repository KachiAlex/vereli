...
export default async function handler(req, res) {
    ...
    try {
      ...
      if (workAreaId) {
        if (user.role === 'superadmin' && !req.query.tenantId) {
          rows = await sql`SELECT id, work_area_id, item, status, created_at FROM approvals WHERE work_area_id = ${Number(workAreaId)} ORDER BY created_at DESC`;
        } else {
          rows = await sql`SELECT id, work_area_id, item, status, created_at FROM approvals WHERE tenant_id = ${tid} AND work_area_id = ${Number(workAreaId)} ORDER BY created_at DESC`;
        }
      } else {
        if (user.role === 'superadmin' && !req.query.tenantId) {
          rows = await sql`SELECT id, work_area_id, item, status, created_at FROM approvals ORDER BY created_at DESC`;
        } else {
          rows = await sql`SELECT id, work_area_id, item, status, created_at FROM approvals WHERE tenant_id = ${tid} ORDER BY created_at DESC`;
        }
      }
    } catch (err) {
      console.error('Error fetching approvals:', err);
      sendJson(res, 500, { error: 'Failed to fetch approvals' });
      return;
    }
  ...
  if (req.method === 'POST') {
    if (!canManageData(user)) {
      sendJson(res, 403, { error: 'Insufficient permissions to create approvals' });
      return;
    }
    ...
    const [row] = await sql`
      INSERT INTO approvals (tenant_id, user_id, work_area_id, item, status)
      VALUES (${tenantId}, ${user.userId}, ${Number(workAreaId)}, ${item}, ${status})
