...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { workAreaId, name, type = 'document', size, visibility = 'internal', uploaderName, url } = req.body || {};
    ...
    const [row] = await sql`
      INSERT INTO files (tenant_id, user_id, work_area_id, name, type, size, visibility, uploader_name, url)
      VALUES (${tenantId}, ${user.userId}, ${Number(workAreaId)}, ${name}, ${type}, ${size || null}, ${visibility}, ${uploaderName || null}, ${url || null})
      RETURNING id, work_area_id, name, type, size, visibility, uploader_name, url, created_at;
    `;
    sendJson(res, 201, { data: { id: row.id, workAreaId: row.work_area_id, name: row.name, type: row.type, size: row.size, visibility: row.visibility, uploaderName: row.uploader_name, url: row.url, createdAt: row.created_at } });
    return;
  }
