...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    ...
    const query = user.role === 'superadmin'
      ? `UPDATE files SET ${fields.join(', ')} WHERE id = $${fields.length + 1} RETURNING id, work_area_id, name, type, size, visibility, uploader_name, created_at`
      : `UPDATE files SET ${fields.join(', ')} WHERE id = $${fields.length + 1} AND tenant_id = $${fields.length + 2} RETURNING id, work_area_id, name, type, size, visibility, uploader_name, created_at`;
    values.push(id);
