...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const [row] = await sql`SELECT id, client_id, name, status, budget, tasks_total, tasks_pending, created_at FROM projects WHERE id = ${id}`;
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    ...
    const [row] = await sql`UPDATE projects SET ${sql.join(updates, sql`, `)} WHERE id = ${id} RETURNING id, client_id, name, status, budget, tasks_total, tasks_pending, created_at`;
    if (!row) { notFound(res); return; }
    sendJson(res, 200, { data: { id: row.id, clientId: row.client_id, name: row.name, status: row.status, budget: row.budget, tasksTotal: row.tasks_total, tasksPending: row.tasks_pending, createdAt: row.created_at } });
    return;
  }
