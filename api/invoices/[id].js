...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const [row] = await sql`SELECT id, client_id, project_id, amount, currency, status, due_date, created_at FROM invoices WHERE id = ${id}`;
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    ...
    const [row] = await sql`UPDATE invoices SET ${sql.join(updates, sql`, `)} WHERE id = ${id} RETURNING id, client_id, project_id, amount, currency, status, due_date, created_at`;
    if (!row) { notFound(res); return; }
    sendJson(res, 200, { data: { id: row.id, clientId: row.client_id, projectId: row.project_id, amount: row.amount, currency: row.currency, status: row.status, dueDate: row.due_date, createdAt: row.created_at } });
    return;
  }
  ...
  if (req.method === 'DELETE') {
    const [row] = await sql`DELETE FROM invoices WHERE id = ${id} RETURNING id`;
    if (!row) { notFound(res); return; }
